...
sortped <- function(ped, addgen = TRUE, ...) {
  ...
  ped_trace_gen_dt[(TraceGenInd == 0) & ((!is.na(Sire)) | (!is.na(Dam))),
                 TraceGenInd := pmin(TraceGenSire, TraceGenDam, na.rm = TRUE) - 1]
  ...
  # Isolated individuals (no parents, no progeny) are assigned a special TraceGenInd
  # which will be converted to Gen = 0 later.
  # We use a temporary marker value (e.g., max_trace_gen_num_s + 999) to identify them
  # before the final conversion.
  isolated_marker <- max_trace_gen_num_s + 999
  ped_trace_gen_dt[(TraceGenInd == 0) & (is.na(Sire) & is.na(Dam)), TraceGenInd := isolated_marker]
  ...
  ped_trace_gen_dt[!is.na(TraceGenSire) | !is.na(TraceGenDam),
              TraceGenInterval := pmin(TraceGenSire, TraceGenDam, na.rm = TRUE) - TraceGenInd]
  ...
  is_isolated <- ped_trace_gen_dt$TraceGenInd == (isolated_marker + 1)
  
  # Calculate max generation excluding isolated individuals
  valid_trace_gens <- ped_trace_gen_dt$TraceGenInd[!is_isolated]
  max_trace_gen <- if(length(valid_trace_gens) > 0) max(valid_trace_gens, na.rm = TRUE) else 1
  
  # Convert the tracing generation to real generation
  ped_trace_gen_dt[, Gen := (-1) * TraceGenInd + max_trace_gen + 1]
  
  # Set Gen = 0 for isolated individuals
  ped_trace_gen_dt[is_isolated, Gen := 0L]
