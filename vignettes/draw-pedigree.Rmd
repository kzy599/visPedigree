...
The inbreeding coefficient (F) of each individual can be calculated and displayed in the pedigree graph. The calculation happens before plotting, and `showf` only controls display:
...
2. Or call `inbreed()` directly on a tidied pedigree to add the `f` column. Then set `showf = TRUE` in `visped()` to append the coefficients to the labels (e.g., "ID\n0.05").
...
# Option 2: Show in the graph (requires existing f column)
