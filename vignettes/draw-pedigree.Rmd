...
4.1.1 [Highlighting specific individuals](#4.1.1)   
4.1.2 [Showing inbreeding coefficients](#4.1.2)   
...
#### <a id="4.1.1">4.1.1 Highlighting specific individuals</a>
...
You can provide a character vector of individual IDs to use the default highlight colors (orange border and light orange fill):
```{r highlight1, fig.width=6.5, fig.height=6.5}
...
```{r highlight2, fig.width=6.5, fig.height=6.5}
...
#### <a id="4.1.2">4.1.2 Showing inbreeding coefficients</a>
The inbreeding coefficient (F) of each individual can be calculated and displayed in the pedigree graph. There are two ways to do this:
...
2. Set `showf = TRUE` in the `visped()` function. This will calculate the inbreeding coefficients (if not already present) and append them to the individual labels in the graph, e.g., "ID (F=0.05)".
...
# Option 2: Calculate and show in the graph
visped(tidy_test, showf = TRUE)
```
